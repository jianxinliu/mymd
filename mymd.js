...
function compileMd(text = "") {
  ...
  for (let i = 0; i < rows.length; ) {
    ...
    if (RegExps.isTitle.test(row)) {
      ...
      let liList = [];
      while (RegExps.isTaskList.test(rows[i])) {
        const c = rows[i++].match(RegExps.isTaskList);
        if (c) {
          let checkStr = c[1].trim();
          let check = checkStr === "X" || checkStr === "x";
          liList.push(
            `<input type="checkbox" ${check ? 'checked' : ''}></input>
            <label>${simpleRow(c[2])}</label>`
          );
        }
      }
      i--;
      rowsRet.push(liList.join("<br>"));
      ...
      i--;
      rowsRet.push(
        `<ol style="list-style: decimal; margin-left: 0px">${liList.join(
          ""
        )}</ol>`
      );
      ...
      i--;
      rowsRet.push(
        `<ul style="list-style: disc; margin-left: 0px">${liList.join("")}</ul>`
      );
    } else {
